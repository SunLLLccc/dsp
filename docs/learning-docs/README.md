...
> 这是 DSP 数据服务平台的学习文档前门，由 learn-project 的 `lp-*` 系列 skill 自动生成。
> 打开这份 README，你应该能在 30 秒内知道：这是什么项目、有哪些功能学习文档、该按什么顺序读。

## 项目一句话简介

DSP 是一个"**零代码发布接口**"的数据服务平台——业务方不再写 Controller / Service / Mapper，而是用一份 XML 描述接口的全部逻辑（入参、查询、数据源、字段映射、响应拼装），由平台在运行时把这份 XML 解释成一个可调用的 HTTP 接口，并集中承载鉴权、审批、审计、缓存、并行编排等横切关注点。
...
| 字段 | 值 | 来源 |
|------|------|------|
| 项目名称 | DSP 数据服务平台 | `_meta/progress.json` 的 `projectName` |
