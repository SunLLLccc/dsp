...
> 目标读者：有经验的 Java/Spring 开发者，但不熟悉 DSP 平台。本文假设你已熟练掌握 Java 8、Spring DI、DOM4J、CompletableFuture；只讲 DSP 特定的设计决策，不讲通用语法。
>
...
DSP（数据服务平台）的核心定位是"**零代码发布接口**"——业务方不再写 Controller / Service / Mapper，而是用一份 XML 描述接口的全部逻辑（入参、查询、数据源、字段映射、响应拼装），由平台在运行时把这份 XML 解释成一个可调用的 HTTP 接口。承担这个"解释并执行"职责的，就是 **`XmlEngine`**——整个 DSP 平台的执行内核。

> 产品价值定位见 `README.md` 第 103 行，核心特性首条：
