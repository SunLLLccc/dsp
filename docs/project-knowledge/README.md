...
DSP 是一个**面向企业查询型数据接口的 AI 辅助设计、受控生成与全生命周期治理平台**。它把自然语言数据需求转化为有表结构依据、受模板约束、可验证、可审批/发布、可授权和可审计的数据接口；XML DSL 与 DAG 查询编排是运行时执行底座，而不是产品的第一卖点。
...
- **核心自封装**：`dsp-engine` 模块的 **XmlEngine**——支持 mysql/doris/oracle/postgresql/http/dubbo/mongo 七种查询类型 + DAG 依赖编排，是查询型接口的执行底座。
- **AI 生成链路**：`dsp-admin-service` 中的 AI 助手与 Text2API 支持项目问答、Schema Evidence 门禁、模板选择、XML/JSON 生成和导入发布。
- **技术特征**：Spring Boot 3.5.16（Java 21）+ MyBatis-Plus + dynamic-datasource 多数据源 + Dubbo + DOM4J/EasyExcel + AgentScope Java；前端 Vue 3.5 + Element Plus + Pinia + Vite。
...
### 我要把项目跑起来（开发环境）
- 读 [05-001-构建打包部署.md](05-001-构建打包部署.md) 的「一、环境要求」（JDK 21 / Node 20 / MySQL 5.7）→「二、依赖安装」→「三、开发启动」三节
...
3. **红线**：改引擎逻辑必须充分手动验证（03 红线 3），因为引擎是查询型接口执行底座，回归面广
