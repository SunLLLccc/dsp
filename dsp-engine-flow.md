...
graph TB
    ...
    SCHED["CacheRefreshScheduler<br/>定时刷新5min"] -.-> CACHE
    INVALID["CacheInvalidator<br/>审批/发布时失效"] -.-> CACHE
...
`XmlConfigCacheManager` 以 `transno` 为 key 缓存已解析的 `InterfaceConfig`。
...
flowchart TD
    ...
    LOOP --> DS{"datasource?"}
    DS -->|是| PDS["parseDataSource()<br/>→ DataSourceConfig<br/>  name/type/url/username/password"]
    ...
    RSP -->|是| PRSP["parseResponseData()<br/>→ List&lt;ResponseFieldConfig&gt;<br/>  name/mapTo/function"]
...
flowchart TD
    ...
    subgraph "响应字段组装"
        ...
        FN_CHECK -->|否| RESP_PUT["response.put(field.name, value)"]
        APPLY_FN --> RESP_PUT
    end
...
<interface transno="TRAN001">
  ...
  <queries>
    ...
    <query id="q2" type="http" datasource="http_ds">
      <url>http://api.internal/user/${requestData.userId}/detail</url>
      <method>GET</method>
  ...
  <!-- 结果映射 -->
  <resultMap>
    <field source="q1.id" name="orderId"/>
    <field source="q1.amount" name="amount" function="fn:ROUND,2"/>
    <field source="q1.create_time" name="createTime" function="fn:DATE_FORMAT,yyyy-MM-dd HH:mm:ss"/>
    <field source="q2.name" name="userName"/>
    <field source="q2.avatar" name="avatarUrl" function="fn:IFNULL,https://default.avatar"/>
  </resultMap>

  <!-- 响应组装 -->
  <responseData>
    <field name="orders" mapTo="q1"/>
    <field name="userInfo" mapTo="q2"/>
    <field name="total" mapTo="q3" function="fn:COUNT"/>
  </responseData>
</interface>
```
